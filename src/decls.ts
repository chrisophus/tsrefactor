// What a declaration is, and how the file's text maps onto declarations.
// Pure syntax: this module knows nothing about a build, which is what lets the
// builder import it without the two modules importing each other.
//
...
function testBlocks(stmt: ExpressionStatement, outerNames: string[], outerKeys: string[], make: MakeDecl): Decl[] {
  ...
  block.parentKey = outerKeys.at(-1);
  const out = [block];
...
function qualify(rel: string, symbol: string): string {
  return `${rel}:${symbol}`;
}

// overlaps reports whether a declaration covers any part of a changed span.
function overlaps(d: Decl, r: LineRange): boolean {
...
export function selectedBy(d: Decl, ranges: readonly LineRange[]): boolean {
  if (ranges.every((r) => !overlaps(d, r))) {
    return false;
  }
  ...
  for (const r of ranges) {
    for (let line = Math.max(r.start, d.start); line <= Math.min(r.end, d.end); line++) {
      if (d.members.every((m) => !(m.start <= line && line <= m.end))) {
        return true;
      }
    }
  }
...
export function declCompare(a: Decl, c: Decl): number {
  ...
  if (a.symbol < c.symbol) return -1;
  if (a.symbol > c.symbol) return 1;
  return 0;
}

// memberKind names what a class member is.
function memberKind(m: Node): string {
  if (Node.isMethodDeclaration(m)) return "method";
  if (Node.isGetAccessorDeclaration(m)) return "getter";
  if (Node.isSetAccessorDeclaration(m)) return "setter";
  return "property";
}
